      ...
      <Sidebar className="h-full border-r bg-gradient-to-b from-background to-background/95 backdrop-blur-sm">
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton className="h-full" asChild>
                <Link href="/dashboard">
                  <div className="flex aspect-square size-10 items-center justify-center rounded-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-md">
                    <Sparkles className="size-5" />
                  </div>
                  <div className="flex flex-col gap-0.5 leading-none">
                    <span className="font-bold text-lg tracking-tight">OMKAR GOLD COVERING</span>
        ...
        <SidebarContent className="px-2 py-4">
          <div className="mb-2 px-3">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Main Menu</p>
          </div>
          <SidebarMenu>
            {filteredLinks.map((link) => (
              <SidebarMenuItem key={link.label}>
                {link.subItems ? (
                  <>
                    <SidebarMenuButton className="justify-between group hover:bg-accent/50 rounded-md">
                      <div className="flex items-center">
                        <div className={`mr-3 p-2 rounded-md ${pathname.startsWith(link.href) ? 'bg-primary/10 text-primary' : 'bg-muted/50 text-muted-foreground group-hover:text-foreground'}`}>
                  ...
                  >
                    <Link href={link.href}>
                      <div className={`mr-3 p-2 rounded-md ${pathname === link.href ? 'bg-primary/10 text-primary' : 'bg-muted/50 text-muted-foreground group-hover:text-foreground'}`}>
                        <link.icon className="h-4 w-4" />
                      </div>
                      <span>{link.label}</span>
                      {link.badge && (
      ...
      <div className="flex flex-col h-full overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-4 ">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex flex-col">
              <span className="font-semibold text-lg tracking-tight">OMKAR GOLD COVERING</span>
              <span className="text-xs text-muted-foreground">
                {isAdmin
                  ? 'Admin Dashboard'
                  : isShopkeeper
              ...
              <div key={link.label} className="mb-1">
                {link.subItems ? (
                  <div className="space-y-1">
                    ...
                    >
                      <div className="flex items-center">
                        <div className={`mr-3 p-2 rounded-md ${pathname.startsWith(link.href) ? 'bg-primary/10 text-primary' : 'bg-muted/50 text-muted-foreground'}`}>
                          <link.icon className="h-4 w-4" />
                        </div>
                        <span>{link.label}</span>
                      </div>
                  ...
                  >
                    <Link href={link.href} onClick={toggleMobileMenu}>
                      <div className={`mr-3 p-2 rounded-md ${pathname === link.href ? 'bg-primary/10 text-primary' : 'bg-muted/50 text-muted-foreground'}`}>
                        <link.icon className="h-4 w-4" />
                      </div>
                      <span>{link.label}</span>
                      {link.badge && (
